    ...
    int opt_verbose)
{
  ...
  PyArrayObject * _frames;
  PyArrayObject * _descriptors;
...
//			}
    ...
    // frames
    _frames = (PyArrayObject*)PyArray_Empty(2, dims, PyArray_DescrFromType(PyArray_DOUBLE), true);
    std::copy(frames, frames + 4 * dims[1], reinterpret_cast<double*>(_frames -> data));
    delete[](frames);
    // The following version that avoids copying was originally enabled.
    // With MSVC11, however, the functions only work if numpy may deallocate
    // self allocated memory.
    //_frames = (PyArrayObject*) PyArray_NewFromDescr(
    ...
    // descriptors
    dims[0] = 128;
    _descriptors = (PyArrayObject*) PyArray_Empty(2, dims, PyArray_DescrFromType(PyArray_UBYTE), true);
    std::copy(descr, descr + 128 * dims[1], reinterpret_cast<unsigned char*>(_descriptors -> data));
    delete[](descr);
    //_descriptors = (PyArrayObject*) PyArray_NewFromDescr(
    //  &PyArray_Type, PyArray_DescrFromType(PyArray_UBYTE),
    //  2, dims, NULL, (unsigned char*)descr, NPY_F_CONTIGUOUS, NULL);
